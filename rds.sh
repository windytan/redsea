...
amixer -q sset 'Capture' 60%
amixer -q sset 'Capture' cap

# 1. sox:       Read /dev/dsp at 48000
# 2. sox:       Filter out FM stereo pilot tone at 19000
# 3. sox:       Bandpass upper sideband of RDS signal
# 4. downmix:   Downmix 19000 -> 0
# 5. sox:       Lowpass & downsample 48000 -> 6000
# 6. bits:      Demodulate BPSK data
# 7. lcd.pl:    Decode RDS data and display it
sox -q -r 48000 -b 16 -t alsa hw:0,0 -c 1 -t raw -b 16 -e signed-integer - \
  sinc -n 4096 -L 19000 sinc 19300-21200 | \
./downmix | \
