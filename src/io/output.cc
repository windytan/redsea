...
/**
 * Format the raw group data as hex, like in RDS Spy.
 *
 * Invalid blocks are replaced with "----".
 *
 */
std::string formatHex(const Group& group) {
  return (group.has(BLOCK1) ? getHexString(group.get(BLOCK1), 4) + " " : "---- ") +
         (group.has(BLOCK2) ? getHexString(group.get(BLOCK2), 4) + " " : "---- ") +
