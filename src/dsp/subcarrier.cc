...
Subcarriers::Subcarriers(float samplerate)
  ...
  for (auto& demod : demods_) {
    demod.agc_.init(kAGCBandwidth_Hz / kTargetSampleRate_Hz, kAGCInitialGain);
    demod.fir_lpf_.init(255, kLowpassCutoff_Hz / kTargetSampleRate_Hz);
    demod.symsync_.init(LIQUID_FIRFILT_RRC, kSamplesPerSymbol, kSymsyncDelay, kSymsyncBeta, 32);
    demod.symsync_.setBandwidth(kSymsyncBandwidth_Hz / kTargetSampleRate_Hz);
    demod.symsync_.setOutputRate(1);
  }
...
BitBuffer Subcarriers::processChunk(MPXBuffer& input_chunk, int num_streams) {
  ...
  for (size_t i = 0; i < chunk.used_size; i++) {
    for (int n_stream{0}; n_stream < num_streams; n_stream++) {
      ...
      demods_[n_stream].fir_lpf_.push(sample_baseband);

      if (sample_num_ % decimate_ratio == 0) {
        std::complex<float> sample_lopass =
            demods_[n_stream].agc_.execute(demods_[n_stream].fir_lpf_.execute());

        const auto symbol = demods_[n_stream].symsync_.execute(sample_lopass);
        ...
        if (symbol.valid) {
          ...
          const auto biphase = demods_[n_stream].biphase_decoder_.push(symbol.data);

          // One biphase symbol received for every 2 PSK symbols
          if (biphase.valid) {
            bitbuffer.bits[n_stream].push_back(
                demods_[n_stream].delta_decoder_.decode(biphase.data));
          }
        }
      }
    }  // for n_stream
