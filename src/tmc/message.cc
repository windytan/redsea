...
std::vector<FreeformField> getFreeformFields(const std::array<MessagePart, 5>& parts) {
  ...
  const auto second_gsi = getBits(parts[1].data[0], 12, 2);
...
void Message::pushMulti(std::uint16_t x, std::uint16_t y, std::uint16_t z) {
  const auto new_continuity_index = getBits(x, 0, 3);
  ...
  if (is_first_group) {
    ...
    const auto group_sequence_indicator = getBits(y, 12, 2);
    current_group                       = 1;
...
void Message::decodeMulti() {
  ...
  extent_    = getBits(parts_[0].data[0], 11, 3);
  events_.push_back(getBits(parts_[0].data[0], 0, 11));
  if (is_encrypted_)
